...
// Luuk 01-02-2021 - Only changed slightly by me
...
const Engine = class {
  constructor(time_step, update_fn, render_fn) {
    ...
    this.handleRun = (time_stamp) => { this.run(time_stamp); } // time_stamp is passed by requestAnimationFrame
  }

  setSlowMo(set_true=true) {
    if (this.slowmo && !set_true) { // turn off
      this.slowmo = false
      this.changeTimeStep(null, true, false) // revert to old_time_step
    } else if (!this.slowmo && set_true) { // turn on
      this.slowmo = true
      this.changeTimeStep(this.slowmo_speed, false, true) // save current speed to old_time_step
    }
  }

  changeTimeStep(value, back=false, save=true) {
    if (back) {
      this.time_step = this.old_time_step
    }
    else {
      if (save) this.old_time_step = this.time_step
